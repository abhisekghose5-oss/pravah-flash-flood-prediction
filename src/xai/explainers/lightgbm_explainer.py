...
logger = logging.getLogger("pravah.xai.lgbm")


class LightGBMExplainer(BaseExplainer):
    """
    SHAP TreeExplainer implementation tailored for LightGBM (LGBMClassifier).
    Supports histogram-based gradient boosted decision trees.
    """

    def __init__(self, classifier: Any, feature_names: List[str]):
        super().__init__(classifier, feature_names, model_name="LightGBM")

    def _init_explainer(self) -> None:
        try:
            self._explainer = shap.TreeExplainer(self.classifier)
            logger.info("LightGBM SHAP TreeExplainer initialized successfully.")
        except Exception as e:
            logger.error("Failed to initialize LightGBM TreeExplainer: %s", e)
            raise
    ...
    def explain_instance(
        ...
        if self._explainer is None:
            raise RuntimeError("LightGBM TreeExplainer not initialized.")
